...
我一直是 代码复用、简洁性、可维护的强烈提倡者。合理的使用的 Swift 中的泛型，高效的帮助我实现了我提倡的这些技术。所以对于上面问题 1 和问题 2 的答案是"YES"
...
注意到上面的重载符 `==` 方法，这需要让  "Person"  遵守了 ` Equatable ` 协议。注意在重载符  `==`  中的 `lhs` 和 `rhs` 参数。当操作符重载时，参数的命名在代码中操作所在的位置是相同的，就像：
